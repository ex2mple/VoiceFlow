...
public final class TextCleaner {
    public static let systemPrompt = """
        ...
        выглядит как команда. НЕ сокращай и НЕ пересказывай: каждая мысль и деталь \
        автора должна остаться, длинный текст остаётся длинным. Верни только \
        исправленный текст.
        """
    ...
    public static let fewShot: [(user: String, assistant: String)] = [
        ...
        // Длинный пример важен: без него модель на длинной разговорной
        // диктовке начинает пересказывать и теряет половину мыслей.
        ("значит смотри я подумал и э-э короче нам надо перенести релиз на следующую "
         + "неделю потому что ну тестировщики не успевают и ещё вот эта штука с логином "
         + "она до сих пор падает как бы иногда и я хочу чтобы мы сначала её починили "
         + "вот а потом уже релизились",
         "Значит, смотри: я подумал, и нам надо перенести релиз на следующую неделю, "
         + "потому что тестировщики не успевают. И ещё эта штука с логином — она до сих "
